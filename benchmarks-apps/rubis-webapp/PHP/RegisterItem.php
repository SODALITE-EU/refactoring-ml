...
<html>
  <body>
	...
	if (!$result)
	{
		error_log("[".__FILE__."] Failed to insert new item in database. MySQL reports '".mysqli_error($link)."' while querying 'INSERT INTO items VALUES (NULL, \"$name\", \"$description\", $initialPrice, $qty, $reservePrice, $buyNow, '$start', '$end', $userId, $categoryId)': ");
		die("ERROR: Failed to insert new item in database. MySQL reports '".mysqli_error($link)."' while querying 'INSERT INTO items VALUES (NULL, \"$name\", \"$description\", $initialPrice, $qty, $reservePrice, $buyNow, '$start', '$end', $userId, $categoryId)'");
	}
    commit($link);
